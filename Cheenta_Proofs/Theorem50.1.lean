...
theorem subspaceOfDimension
  ...
  unfold Covering_Dimension_General
  intro ι u hu
  ...
  · obtain ⟨κ, _, v, hv_cov, hv_ref, hv_ord⟩ := hdim (Option ι) cover
  ...
  · have hY : IsEmpty Y := ⟨fun y => let ⟨i, _⟩ := Set.mem_iUnion.mp (hu.2.symm ▸ Set.mem_univ y); hι ⟨i⟩⟩
