...
theorem lebesgue_number_lemmaog {X : Type*} [MetricSpace X] (hX : CompactSpace X)
  ...
  -- Extract finite subcover by half-balls
  ...
  -- Handle empty X: s = ∅ forces X to be empty, theorem holds trivially
  ...
  -- Get the minimizer of r(x)/2 over the finite subcover
  -- This replaces Finset.inf' entirely
  obtain ⟨x_min, hx_min_s, hx_min_le⟩ :=
    s.exists_min_image (fun x => r x / 2) hs_nonempty
  -- δ = r(x_min)/2, positive since r(x_min) > 0
  refine ⟨r x_min / 2, half_pos (hr_pos x_min), fun y => ?_⟩
  -- y lands in some half-ball B(xi, r(xi)/2)
  ...
  -- B(y, δ) ⊆ U(xi) by triangle inequality
  refine ⟨U xi, hU xi, fun z hz => ?_⟩
  apply hr_ball xi
  simp only [Metric.mem_ball] at hz ⊢
  -- δ ≤ r(xi)/2 because x_min minimizes over s
